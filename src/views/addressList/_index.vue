        ...
        <div v-loading="treeLoading">
          <el-tree
              ...
              <span slot-scope="{ node, data }" class="custom-tree-node" :ref="`cs-node${data.id}`"
                ...
                <span class="custom-tree-node__item"><i :class="['iconEIP',data.icon]"></i> <span :title="node.label">{{ strSplit(node.label) }}</span></span>
              </span>
          </el-tree>
        </div>
      </div>
...
<script>
  ...
  export default {
    ...
    watch: {
      mailwhitelist(newV){
        if(!newV){
          this.$router.replace(`/addressList?id=${this.$route.query.id}&sid=${this.$route.query.sid}`)
        }
      },
      userName: {
        handler(newV){
    ...
    methods: {
      loadNode(node, resolve) {
        ...
        if (node.level === 1) {
          return resolve(this.treeList[0].children.map(v => {
            return {
              ...v,
              icon: v.orgType === 1 ? "icon_building" : 'icon_dep',
              leaf: !v.children.length
            }
          }))
        }
        if (node.level > 1) {
      ...
      mailOrgTreeForVip() {
        ...
        API.home.mailOrgTreeForVip().then(res => {
      ...
      orgUserList(type) {
        ...
        API.home.usersByOrgCodeForVip(params).then(res => {
      ...
      isOrgCanHit(item) {
        API.home.isOrgCanHit({orgCode: item.code}).then(res => {
          if (res.data.code === 200) {
            this.$set(item, 'canHit', res.data.data ? 1 : 2)
            this.viewData(item)
          }
        })
      },
      viewData(item) {
        if (item.children.length) {
          //有子级列表则要查看children[0].orgType是否等于2,如果是则可以查询右侧列表
          if (item.children[0].orgType == 2) {
            //可以查询
            this.handleGetUserList(item)
          }
        } else {
          //有两种情况children为空,第一种是还没加载子列表,第二种是加载了子列表没数据
          if (item.canHit) {
            //已经调了canhit接口
            if (item.canHit === 1) {
              //能点击查询右侧列表
            ...
            //没调canHit接口
            this.isOrgCanHit(item)
          }
        }
      },
      handleGetUserList(item){
        if (this.selectDom) {
          //先还原原来的颜色
